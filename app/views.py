...
commodity_views = APIRouter()


@commodity_views.get("", response_model=List[Commodity])
async def get_commodities():
    return await get_all()


@commodity_views.post("", response_model=Commodity)
...
@commodity_views.get("/{text}", response_model=List[Commodity])
async def filter_commodities_by_text(text: str):
    return await filter_by(text=text)


@commodity_views.put("/{id}", response_model=Commodity)
